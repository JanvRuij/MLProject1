...
Z = model.addVars(N, nr_nodes, vtype=GRB.CONTINUOUS, lb=0, ub=1, name="Z")
...
model.setObjective(gp.quicksum(aux[i] for i in range(N))
                    + lam * gp.quicksum(A[i, j] * A[i, j] for i in range(N) for j in range(nr_nodes))
                   , GRB.MINIMIZE)
